...
def _generic_no4_adversarial(y_trn, y_insp, hx_qtb, picked, pick_trntst,
    ...
    elegant_print([
        "\t\t Normally (fairness)",
        "\t\t\t Group #1: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_1[0], tmp_1[1], grp_1),
        "\t\t\t Group #2: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_2[0], tmp_2[1], grp_2),
        "\t\t\t Group #3: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_3[0], tmp_3[1], grp_3),
    ], logger)
    ...
    elegant_print([
        ...
        "\t\t\t Group #1: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_1[0], tmp_1[1], adv_1),
        "\t\t\t Group #2: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_2[0], tmp_2[1], adv_2),
        "\t\t\t Group #3: {:.6f} {:.6f}  abs: {:.6f}".format(tmp_3[0], tmp_3[1], adv_3),
    ], logger)

    elegant_print([
        "\t\t Discriminative risk (DR)",
        "\t\t\t  hat_loss : {:.12f}".format(hat_L_loss(lab_y_insp, clas_y_trn)),
        "\t\t\t  hat_fair : {:.12f}".format(hat_L_fair(lab_y_insp, lab_hx_qtb)),
...
def pseudo_classification(non_sa_trn, non_sa_tst, picked, logger,
    ...
    train_y, y_insp, hx_qtb_trn, test_y, y_pred, hx_qtb_tst = curr_dat_s

    # pdb.set_trace()
    elegant_print(f"\n{picked} Model:", logger)

    elegant_print(f"\t training, thres= {threshold}, non_sa#1 sex", logger)
    _generic_no4_adversarial(train_y, y_insp, hx_qtb_trn, picked, 'training',
                             logger, threshold, positive_label, non_sa_1)
    elegant_print(f"\t training, thres= {threshold}, non_sa#2 race", logger)
    _generic_no4_adversarial(train_y, y_insp, hx_qtb_trn, picked, 'training',
                             logger, threshold, positive_label, non_sa_2)

    elegant_print(f"\t test, thres= {threshold}, non_sa#1 sex", logger)
    _generic_no4_adversarial(test_y, y_pred, hx_qtb_tst, picked, 'test',
                             logger, threshold, positive_label, non_sa_1_tst)
    elegant_print(f"\t test, thres= {threshold}, non_sa#2 race", logger)
    _generic_no4_adversarial(test_y, y_pred, hx_qtb_tst, picked, 'test',
                             logger, threshold, positive_label, non_sa_2_tst)

...
class PartI_LambdaEffect(EnsembleSetup):
    ...
    def calc_simplified_fair_quality(self, y, ys, yr, fens, fqtb,
        ...
        ans_A1 = self.calc_reduced_fair_gather(
            y, fens, pos_val, tag[0])
        if not jt:
            ans_A2 = [''] * 10
            ans_Jt = [''] * 10
            return tmp, ans_A1, ans_A2, ans_Jt
        ans_A2 = self.calc_reduced_fair_gather(
            y, fens, pos_val, tag[1])
        ans_Jt = self.calc_reduced_fair_gather(
            y, fens, pos_val, jt[0])
...
class PartE_ImprovedFairness(EnsembleSetup):
    ...
    def prepare_trial(self):
        ...
        csv_row_3c_bc = [
            'Erho^2[Lfair(f,fp)]', 'Erho^2[Lloss(f,fp)]',

...
def test_early_stopping():
    from fairml.facilc.ensem_pruning import Early_Stopping
...
def test_KL_divergence():
    ...
    from fairml.facils.data_entropy import prob
    px, _ = prob(tr_ha)
    py, _ = prob(tr_hb)
...
def test_error_reduce():
    from fairml.facilc.ensem_pruning import Reduce_Error_Pruning
...
def test_GMA_diversity():
    from fairml.facilc.ensem_pruning import _GMM_Kappa_sum, GMM_Algorithm
...
def test_drep_multi_modify():
    ...
    from fairml.facilc.ensem_pruning import DREP_Pruning
...
def test_pep_OEP_SEP():
    ...
    mu_yo, mu_P, mu_seq = PEP_OEP(mu_y, mu_yt, nb_cls)
    assert 1 <= sum(tr_P) == len(tr_seq) < nb_cls
    assert 1 <= sum(bi_P) == len(bi_seq) < nb_cls
    assert 1 <= sum(mu_P) == len(mu_seq) < nb_cls
...
def test_pep_VDS_PEP():
    from fairml.facilc.ensem_pruning import _PEP_VDS, PEP_PEP
    s = np.random.randint(2, size=nb_cls, dtype='bool').tolist()
...
def test_contrastive():
    from fairml.facilc.ensem_pruning import contrastive_pruning_methods

...
class BaseWeightBoosting(six.with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.estimators_raw = self.estimators_  # self-made (the next two lines)
        self.estimators_wgt = self.estimator_alphas_ / sum(self.estimator_alphas_)
...
class AdaFair(BaseWeightBoosting, ClassifierMixin):
    ...
    def _boost_discrete(self, iboost, X, y, sample_weight, random_state):
        ...
        alpha = 1 * (
            ...
            np.log((1. - estimator_error) / check_zero(estimator_error)) +
            np.log(n_classes - 1.))
        if estimator_error <= 0.:
            alpha = 1.
        ...
        if self.use_validation:
            ...
            # TPR = (float(tp)) / (tp + fn)
            # TNR = (float(tn)) / (tn + fp)
            TPR = float(tp) / check_zero(tp + fn)
            TNR = float(tn) / check_zero(tn + fp)

...
from fairml.facil.data_entropy import I as EntropyI
gc.enable()
...
def contrastive_pruning_lately(name_pru, nb_cls, nb_pru,
    ...
    # \citep{zhang2019two}
        ...
        P, _, seq = TwoStagePruning_APplusDP(
            y_trn, y_insp, nb_cls, ta=td, td=ta, y_cast=y_cast, **kwargs)
...
def contrastive_pruning_lately_validate(name_pru, nb_cls, nb_pru,
                                        y_trn, y_val, y_insp, y_cast,
                                        y_pred, coef, clfs,
                                        alpha=0.5, L=4, R=3, **kwargs):
    since = time.time()
